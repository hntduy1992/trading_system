...
"""
        ...
        payload = {
            "model": self.model_name,
            "messages": [
                {"role": "system", "content": "You are a quantitative trading risk gatekeeper specializing in Lance Beggs Price Action. Reply with strict JSON only."},
                {"role": "user", "content": prompt}
            ],
            "response_format": {"type": "json_object"}
        }
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                return PreEntryEvaluation(
                    ...
                    reason=str(r.get("reason", f"Phê duyệt bởi {self.model_name}")),
            ...
            print(f"[OpenAIAdapter] Pre-entry evaluation API failed: {e}. Falling back to deterministic engine.")
...
"""
        ...
        payload = {
            "model": self.model_name,
            "messages": [
                {"role": "system", "content": "You are a quantitative trading auditor specializing in Lance Beggs Price Action. Reply with strict JSON only."},
                {"role": "user", "content": prompt}
            ],
            "response_format": {"type": "json_object"}
        }
        ...
        try:
            ...
            print(f"[OpenAIAdapter] Audit API call failed: {e}. Falling back to deterministic engine.")

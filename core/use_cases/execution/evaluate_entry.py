...
class EvaluateEntryUseCase:
    ...
    async def execute(
        ...
        Optimized for Live MT5 execution.
        """
        ...
        # 3. Detect 1m Stall Micro-structure (flexible fallback)
        ...
        # 4. Check enabled setups according to session config
        for setup_name, enabled in config.setups_enabled.items():
            ...
            if triggered and side and pullback_price and t1 and t2:
                ...
                print(f"[ENGINE] Trade Executed: {trade_id} [{setup_name} {side.value} {order_type} ticket={order_ticket}]")
                return lifecycle
